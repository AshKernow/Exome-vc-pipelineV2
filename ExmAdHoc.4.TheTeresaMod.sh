...
# This causes problems with the bam2fq-->bwa pipeline - the bam2fq does not recoginise the pairs and treats bwa then sees them as all singletons.
...
if [[ /bin/hostname==*.hcp ]]; then
source /etc/profile.d/sge.sh
source $HOME/.bash_profile
fi

if [[ -z $OutBam ]]; then
	OutBam=`basename $InpBam`
	OutBam=${OutBam/.bam/.fixed.bam}
fi
